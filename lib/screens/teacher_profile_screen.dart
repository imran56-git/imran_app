...
import 'location_picker_screen.dart'; // Ensure this file exists in your project
...
class _TeacherProfileScreenState extends State<TeacherProfileScreen> {
  ...
  final List<String> allSubjects = [
    ...
    'Higher Math 1st/2nd Paper', 'Statistics', 'Management', 'Marketing', 
    'Production Management', 'Commercial Law', 'Logic', 'Sociology', 'Social Work', 
    'Psychology', 'Islamic History', 'Philosophy', 'Political Science', 'Sanskrit', 'Home Science',
    ...
    'Software Engineering', 'Textile Engineering', 'Anatomy', 'Physiology', 
    'Biochemistry', 'Microbiology', 'Pharmacy', 'BBA', 'MBA', 'Human Resource (HRM)', 
    'Supply Chain Management', 'Chartered Accountancy (CA)', 'Public Speaking', 'Legal Studies (Law)',
  ...
  void _openMapPicker() async {
    ...
    if (results != null && results.isNotEmpty) {
      setState(() {
        // Appends newly picked locations from the map to the existing list
  ...
  Future<void> updateTeacherProfile() async {
    ...
    try {
      ...
      await _firestore.collection('teachers').doc(uid).update({
        ...
        'locations': teacherLocations, // Saves the structured Map list
      });
  ...
  Widget _buildViewProfile() {
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8, 
          children: teacherLocations.map((l) {
            String label = (l is Map) ? (l['address'] ?? "Unknown") : l.toString();
            return Chip(label: Text(label), avatar: const Icon(Icons.location_on, size: 16));
          }).toList(),
        ),
  ...
  Widget _buildEditForm() {
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          children: teacherLocations.map((loc) {
            String label = (loc is Map) ? (loc['address'] ?? "Unknown") : loc.toString();
            return Chip(
              label: Text(label),
        ...
        TextButton.icon(
          ...
          label: const Text("Pick Locations from Map"),
        ),
        const Divider(),
